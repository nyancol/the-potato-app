...
def generate_zodiac(env, first_name, last_name, communication_style, interests, personality, role_in_group,
    ...
    You must generate a JSON with 5 string fields: 'love', 'money_and_work', 'health_and_fitness', 'family_and_friends', 'image_description'.
    In the fields 'love', 'money_and_work', 'health_and_fitness', 'family_and_friends' create the horosope for that person for these categories.
    For the field 'image_description' you describe an artistic and alegorical image of that person represented by the his or her zodiac sign
    ('{zodiac_sign}') and the predicted horoscope.
    Be only descriptive of the image without commenting on the meaning, in a way it could be interpreted by a text-to-image AI,
    ...
    completion = client.chat.completions.create(
        ...
        messages=[
        {"role": "system", "content": "You are a poetic assistant, skilled in seeing the future and people horoscope, which outputs in JSON."},
...
def load_potatoes(env, year_range):
    ...
    df_personalities = DeltaTable(potato_description_path).to_pandas(columns=["first_name", "last_name", "communication_style",
    ...
    df = df[["first_name", "last_name", "communication_style", "interests", "personality", "role_in_group", "memorable_stories",
              "plans_aspirations", "year_range", "birth_date"]]
...
if __name__ == "__main__":
    ...
    for card in load_potatoes(env, year_range):
        ...
        zodiac = generate_zodiac(env, card["first_name"], card["last_name"], card["communication_style"], card["interests"],
                                 card["personality"], card["role_in_group"], card["memorable_stories"], card["plans_aspirations"], 
                                 zodiac_sign, existing_zodiacs)
